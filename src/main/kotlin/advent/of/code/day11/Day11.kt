...
fun main() {
    val result = Either.fx<Error, List<Long>> {
        ...
        val robot = Robot(program + additionalMemory, 1, input, output, null)

        val panelsSize = 100
        val panelsField = List(panelsSize) { List(panelsSize) { listOf(0) } }
        val startingPosition = Pair(panelsSize / 2, panelsSize / 2)

        GlobalScope.worker(panelsField, startingPosition, output, input)

        val result = robot.executeOperationsTailRec().bind()
...
fun CoroutineScope.worker(
    panelsField: List<List<List<Int>>>,
    startingPosition: Pair<Int, Int>,
    receiveChannel: ReceiveChannel<Long>,
    sendChannel: SendChannel<Long>
) = this.launch {
    val state: MutableList<Tuple4<List<List<List<Int>>>, Int, Int, Direction>> = mutableListOf(
        Tuple4(
            panelsField,
    ...
    while (!receiveChannel.isClosedForReceive) {
        val color = receiveChannel.receive()
        val directionNumber = receiveChannel.receive().toInt()
        val lastState = state.last()
        val newField = lastState.a.replaceAtIndex2(
            lastState.b,
            lastState.c,
            lastState.a[lastState.c][lastState.b] + color.toInt()
        )
        val newDirection = lastState.d.rotate(directionNumber)
        val (newPositionX, newPositionY) = newDirection.evaluateNewPosition(lastState.b, lastState.c)
        val newState = Tuple4(newField, newPositionX, newPositionY, newDirection)

        sendChannel.offer(newField[newPositionY][newPositionX].last().toLong())

        println(newField.flatten().filter { it.size > 1 }.count())
        if (newField.flatten().filter { it.size > 1 }.count() > 248) {
            newField.map { row ->
                row.map { panel ->
...
sealed class Direction {
    ...
    abstract fun evaluateNewPosition(x: Int, y: Int): Pair<Int, Int>
}

object Up : Direction() {
    override fun left(): Direction = Left
    override fun right(): Direction = Right
    override fun evaluateNewPosition(x: Int, y: Int): Pair<Int, Int> = Pair(x, y + 1)
}
...
object Left : Direction() {
    ...
    override fun evaluateNewPosition(x: Int, y: Int): Pair<Int, Int> = Pair(x - 1, y)
}
...
fun Robot.executeOperationsTailRec(): Either<Error, List<Long>> = Either.tailRecM(Triple(0, 0, program)) {
