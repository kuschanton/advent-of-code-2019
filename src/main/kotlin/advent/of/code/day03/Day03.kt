...
fun main() {
    listOf(
        ...
        .map {
            // Part 1
            findClosestIntersection(it.first, it.second) to
            // Part 2
                    findShortestCircuit(it.first, it.second)
        }
        .forEach { println(it) }
}
...
fun findShortestCircuit(wire1: List<Direction>, wire2: List<Direction>): Either<Error, Int> =
...
fun findWiresIntersections(wire1: List<Direction>, wire2: List<Direction>): List<Point> =
...
fun Point.manhattanDistance(): Int = abs(x) + abs(y)

fun findClosestIntersection(wire1: List<Direction>, wire2: List<Direction>): Either<Error, Point> =
    findWiresIntersections(wire1, wire2)
        .minBy { abs(it.x) + abs(it.y) }
        .toOption()
        .toEither { Error("Wires do not intersect") }

fun List<Direction>.toPoints(): List<Point> =
    this.fold(listOf(Point(0, 0))) { acc, next ->
        val continueFrom = acc.last()
        acc + next.toPoints(continueFrom).drop(1)
    }

fun Direction.toPoints(start: Point): List<Point> =
    when (this) {
...
fun String.toWire(): Either<Error, List<Direction>> =
    this.split(',')
        .map { it.toDirection() }
        .toEitherList()

fun <E, T> List<Either<E, T>>.toEitherList(): Either<E, List<T>> =
    this.fold<Either<E, T>, Either<E, List<T>>>(emptyList<T>().right()) { accEither, nextEither ->
        accEither.flatMap { acc ->
            nextEither.map { next -> acc + next }
        }
    }

fun String.toDirection(): Either<Error, Direction> =
    if (this.isEmpty()) Error("Leg is empty").left()
    else parseLeg(this.first(), this.drop(1))

fun parseLeg(direction: Char, steps: String): Either<Error, Direction> = Either.fx {
    val (directionConstructor) = parseDirection(direction)
    val (stepsNumber) = steps.toIntOrError()
    directionConstructor(stepsNumber)
}

fun parseDirection(direction: Char): Either<Error, (Int) -> Direction> =
    when {
...
fun String.toIntOrError(): Either<Error, Int> =
